...
ace.define('ace/mode/css/csslint', [], (require, exports, module) => {
  ...
  (function () {
    ...
    StringReader.prototype = {
      ...
      readMatch(matcher) {
        let source = this._input.substring(this._cursor),
          value = null;
  ...
  (function () {
    let EventTarget = parserlib.util.EventTarget,
    ...
    function MediaQuery(modifier, mediaType, features, line, col) {
      SyntaxUnit.call(this, (modifier ? `${modifier} ` : '') + (mediaType || '') + (mediaType && features.length > 0 ? ' and ' : '') + features.join(' and '), line, col, Parser.MEDIA_QUERY_TYPE);
    ...
    Parser.prototype = (function () {
      let proto = new EventTarget(), // new prototype
        prop,
        additions = {
          constructor: Parser,
          DEFAULT_TYPE: 0,
          ...
          _stylesheet() {
            let tokenStream = this._tokenStream,
              charset = null,
              count,
              token,
              tt;
    ...
    const Properties = {
      ...
      azimuth(expression) {
        let simple = '<angle> | leftwards | rightwards | inherit',
    ...
    var Validation = {
      ...
      validate(property, value) {
        let name = property.toString().toLowerCase(),
          parts = value.parts,
    ...
    var ValidationTypes = {
      ...
      isLiteral(part, literals) {
        let text = part.text.toString().toLowerCase(),
          args = literals.split(' | '),
          i,
          len,
          found = false;
      ...
      complex: {
        ...
        '<flex>': function (expression) {
          ...
          if (ValidationTypes.isAny(expression, 'none | inherit')) {
            result = true;
          } else if (ValidationTypes.isType(expression, '<flex-grow>')) {
            if (expression.peek()) {
  ...
  function clone(parent, circular, depth, prototype) {
    ...
    function _clone(parent, depth) {
      ...
      if (util.isArray(parent)) {
        ...
        parent.copy(child);
        return child;
      } else if (typeof prototype === 'undefined') child = Object.create(Object.getPrototypeOf(parent));
  ...
  const CSSLint = (function () {
    ...
    api.format = function (results, filename, formatId, options) {
      let formatter = this.getFormatter(formatId),
        result = null;
  ...
  CSSLint.addRule({
    ...
    init(parser, reporter) {
      ...
      parser.addListener('property', (event) => {
        ...
        let propertyName = event.property.toString().toLowerCase(),
  ...
  CSSLint.addRule({
    ...
    init(parser, reporter) {
      ...
      parser.addListener('endrule', (event) => {
        let currentProperties = properties.join(','),
  ...
  CSSLint.addRule({
    ...
    init(parser, reporter) {
      ...
      parser.addListener('property', (event) => {
        let name = event.property.text.toLowerCase(),
          value = event.value;
  ...
  CSSLint.addRule({
    ...
    init(parser, reporter) {
      ...
      parser.addListener('startfontface', startRule);
      parser.addListener('property', (event) => {
        let name = event.property.toString().toLowerCase(),
          value = event.value;
...
ace.define('ace/mode/css_worker', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const lang = require('../lib/lang');
