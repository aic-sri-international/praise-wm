...
ace.define('ace/mode/html/saxparser', [], (require, exports, module) => {
  module.exports = (function e(t, n, r) { function s(o, u) { if (!n[o]) { if (!t[o]) { const a = typeof require === 'function' && require; if (!u && a) return a(o, !0); if (i) return i(o, !0); throw new Error(`Cannot find module '${o}'`); } const f = n[o] = { exports: {} }; t[o][0].call(f.exports, (e) => { const n = t[o][1][e]; return s(n || e); }, f, f.exports, e, t, n, r); } return n[o].exports; } var i = typeof require === 'function' && require; for (let o = 0; o < r.length; o++)s(r[o]); return s; }({
    ...
    2: [function (_dereq_, module, exports) {
      const entities = _dereq_('html5-entities');
      const InputStream = _dereq_('./InputStream').InputStream;

      const namedEntityPrefixes = {};
      ...
      EntityParser.consumeEntity = function (buffer, tokenizer, additionalAllowedCharacter) {
        ...
        if (ch == '#') {
          ...
          if (isDigit(ch)) {
            ...
            const replacement = this.replaceEntityNumbers(code);
            if (replacement) {
            ...
            if (code > 0xFFFF && code <= 0x10FFFF) {
		        code -= 0x10000;
		        const first = ((0xffc00 & code) >> 10) + 0xD800;
		        const second = (0x3ff & code) + 0xDC00;
      ...
      InputStream.prototype = {
        ...
        matchUntil(re) {
          ...
          if (s === InputStream.EOF) {
            ...
            const t = this.data.slice(this.start, this.start + m.index);
    ...
    4: [function (_dereq_, module, exports) {
      ...
      StackItem.prototype.isHtmlIntegrationPoint = function () {
        if (this.namespaceURI === 'http://www.w3.org/1998/Math/MathML') {
          ...
          let encoding = getAttribute(this, 'encoding');
    ...
    5: [function (_dereq_, module, exports) {
      const InputStream = _dereq_('./InputStream').InputStream;
      const EntityParser = _dereq_('./EntityParser').EntityParser;
      ...
      Tokenizer.prototype._emitToken = function (token) {
        if (token.type === 'StartTag') {
          for (let i = 1; i < token.data.length; i++) {
      ...
      Tokenizer.prototype.tokenize = function (source) {
        ...
        function character_reference_in_data_state(buffer) {
          const character = EntityParser.consumeEntity(buffer, tokenizer);
        ...
        function rcdata_end_tag_name_state(buffer) {
          const appropriate = tokenizer._currentToken && (tokenizer._currentToken.name === this._temporaryBuffer.toLowerCase());
          const data = buffer.char();
          if (isWhitespace(data) && appropriate) {
            tokenizer._currentToken = {
              type: 'EndTag', name: this._temporaryBuffer, data: [], selfClosing: false,
            };
        ...
        function script_data_end_tag_name_state(buffer) {
          ...
          if (isWhitespace(data) && appropriate) {
            tokenizer._currentToken = {
              type: 'EndTag', name: 'script', data: [], selfClosing: false,
            };
            tokenizer.setState(before_attribute_name_state);
          } else if (data === '/' && appropriate) {
            tokenizer._currentToken = {
        ...
        function close_tag_open_state(buffer) {
          ...
          if (data === InputStream.EOF) {
            ...
            tokenizer._parseError('expected-closing-tag-but-got-char', { data }); // param 1 is datavars:
            buffer.unget(data);
            tokenizer.setState(bogus_comment_state);
          }
          return true;
        ...
        function attribute_name_state(buffer) {
          ...
          if (leavingThisState) {
            const attributes = tokenizer._currentToken.data;
            const currentAttribute = attributes[attributes.length - 1];
            for (let i = attributes.length - 2; i >= 0; i--) {
        ...
        function character_reference_in_attribute_value_state(buffer) {
          const character = EntityParser.consumeEntity(buffer, tokenizer, this._additionalAllowedCharacter);
        ...
        function markup_declaration_open_state(buffer) {
          ...
          if (chars === '--') {
            ...
            if (chars.toUpperCase() === 'DOCTYPE') {
              tokenizer._currentToken = {
                type: 'Doctype', name: '', publicId: null, systemId: null, forceQuirks: false,
              };
    ...
    6: [function (_dereq_, module, exports) {
      ...
      const messages = _dereq_('./messages.json');
      const constants = _dereq_('./constants');

      const EventEmitter = _dereq_('events').EventEmitter;

      const Tokenizer = _dereq_('./Tokenizer').Tokenizer;
      const ElementStack = _dereq_('./ElementStack').ElementStack;
      const StackItem = _dereq_('./StackItem').StackItem;
      ...
      function TreeBuilder() {
        ...
        const tree = this;
        const modes = this.insertionModes = {};
        modes.base = {
          ...
          processStartTag(name, attributes, selfClosing) {
        ...
        modes.initial.processDoctype = function (name, publicId, systemId, forceQuirks) {
          ...
          } else if ((publicId == '-//W3C//DTD HTML 4.0//EN' && (systemId == null || systemId == 'http://www.w3.org/TR/REC-html40/strict.dtd'))
        ...
        modes.afterFrameset.processCharacters = function (buffer) {
          const characters = buffer.takeRemaining();
          let whitespace = '';
          for (let i = 0; i < characters.length; i++) {
            const ch = characters[i];
        ...
        modes.inHead.processEOF = function () {
          const name = tree.currentStackItem().localName;
        ...
        modes.inHead.processCharacters = function (buffer) {
          const leadingWhitespace = buffer.takeLeadingWhitespace();
        ...
        modes.inBody.processCharacters = function (buffer) {
          ...
          let characters = buffer.takeRemaining();
        ...
        modes.inBody.startTagListItem = function (name, attributes) {
          const stopNames = { li: ['li'], dd: ['dd', 'dt'], dt: ['dd', 'dt'] };
          const stopName = stopNames[name];

          const els = tree.openElements;
          for (let i = els.length - 1; i >= 0; i--) {
            const node = els.item(i);
        ...
        modes.inBody.startTagA = function (name, attributes) {
          const activeA = tree.elementInActiveFormattingElements('a');
          if (activeA) {
        ...
        modes.inBody.startTagInput = function (name, attributes) {
          const currentFramesetOk = tree.framesetOk;
        ...
        modes.inBody.startTagSelect = function (name, attributes) {
          ...
          tree.insertElement(name, attributes);
          tree.framesetOk = false;
          const insertionModeName = tree.insertionModeName;
        ...
        modes.inBody.endTagOther = function (name) {
          let node;
          for (let i = tree.openElements.length - 1; i > 0; i--) {
            node = tree.openElements.item(i);
            if (node.localName == name) {
              tree.generateImpliedEndTags(name);
              if (tree.currentStackItem().localName != name) { tree.parseError('unexpected-end-tag', { name }); }
        ...
        modes.inBody.endTagListItem = function (name) {
          if (!tree.openElements.inListItemScope(name)) {
            ...
            tree.generateImpliedEndTags(name);
            if (tree.currentStackItem().localName != name) { tree.parseError('end-tag-too-early', { name }); }
            tree.openElements.popUntilPopped(name);
          }
        };
        ...
        modes.inCaption.startTagTableElement = function (name, attributes) {
          tree.parseError('unexpected-end-tag', { name });
          const ignoreEndTag = !tree.openElements.inTableScope('caption');
        ...
        modes.inColumnGroup.processCharacters = function (buffer) {
          ...
          if (leadingWhitespace) { tree.insertText(leadingWhitespace); }
          if (!buffer.length) { return; }
          const ignoreEndTag = this.ignoreEndTagColgroup();
        ...
        modes.inForeignContent.processStartTag = function (name, attributes, selfClosing) {
          if (['b', 'big', 'blockquote', 'body', 'br', 'center', 'code', 'dd', 'div', 'dl', 'dt', 'em', 'embed', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'head', 'hr', 'i', 'img', 'li', 'listing', 'menu', 'meta', 'nobr', 'ol', 'p', 'pre', 'ruby', 's', 'small', 'span', 'strong', 'strike', 'sub', 'sup', 'table', 'tt', 'u', 'ul', 'var'].indexOf(name) != -1
				|| (name == 'font' && attributes.some(attr => ['color', 'face', 'size'].indexOf(attr.nodeName) >= 0))) {
        ...
        modes.inForeignContent.processEndTag = function (name) {
          let node = tree.currentStackItem();
          let index = tree.openElements.length - 1;
          if (node.localName.toLowerCase() != name) { tree.parseError('unexpected-end-tag', { name }); }
        ...
        modes.inTable.processCharacters = function (data) {
          if (tree.currentStackItem().isFosterParenting()) {
            const originalInsertionMode = tree.insertionModeName;
        ...
        modes.inTableText.flushCharacters = function () {
          const characters = tree.pendingTableCharacters.join('');
        ...
        modes.inRow.startTagTableOther = function (name, attributes) {
          const ignoreEndTag = this.ignoreEndTagTr();
          this.endTagTr('tr');
          if (!ignoreEndTag) tree.insertionMode.processStartTag(name, attributes);
        };
        ...
        modes.text.processCharacters = function (buffer) {
          ...
          const data = buffer.takeRemaining();
          if (!data) { return; }
          tree.insertText(data);
        };
        ...
        modes.text.endTagScript = function (name) {
          const node = tree.openElements.pop();
      ...
      TreeBuilder.prototype.adoptionAgencyEndTag = function (name) {
        const outerIterationLimit = 8;
        const innerIterationLimit = 3;
        ...
        while (outerLoopCounter++ < outerIterationLimit) {
          ...
          const furthestBlock = this.openElements.furthestBlockForFormattingElement(formattingElement.node);
          ...
          const afeIndex = this.openElements.elements.indexOf(formattingElement);
          const commonAncestor = this.openElements.item(afeIndex - 1);

          let bookmark = this.activeFormattingElements.indexOf(formattingElement);

          let node = furthestBlock;
          let lastNode = furthestBlock;
          let index = this.openElements.elements.indexOf(node);
      ...
      TreeBuilder.prototype.processToken = function (token) {
        ...
        const currentNode = this.openElements.top || null;
        let insertionMode;
      ...
      TreeBuilder.prototype.insertHtmlElement = function (attributes) {
        const root = this.createElement('http://www.w3.org/1999/xhtml', 'html', attributes);
      ...
      TreeBuilder.prototype.insertHeadElement = function (attributes) {
        const element = this.createElement('http://www.w3.org/1999/xhtml', 'head', attributes);
      ...
      TreeBuilder.prototype.insertBodyElement = function (attributes) {
        const element = this.createElement('http://www.w3.org/1999/xhtml', 'body', attributes);
        this.attachNode(element, this.openElements.top.node);
      ...
      TreeBuilder.prototype.insertIntoFosterParent = function (node) {
        const tableIndex = this.openElements.findIndex('table');
        const tableElement = this.openElements.item(tableIndex).node;
      ...
      TreeBuilder.prototype.insertElement = function (name, attributes, namespaceURI, selfClosing) {
        ...
        const element = this.createElement(namespaceURI, name, attributes);
      ...
      TreeBuilder.prototype.generateImpliedEndTags = function (exclude) {
        const name = this.openElements.top.localName;
      ...
      TreeBuilder.prototype.reconstructActiveFormattingElements = function () {
        ...
        let entry = this.activeFormattingElements[i];
        ...
        while (true) {
          ...
          const element = this.currentStackItem();
      ...
      TreeBuilder.prototype.ensureNoahsArkCondition = function (item) {
        ...
        let candidates = [];
        const newItemAttributeCount = item.attributes.length;
      ...
      TreeBuilder.prototype.removeElementFromActiveFormattingElements = function (item) {
        const index = this.activeFormattingElements.indexOf(item);
      ...
      TreeBuilder.prototype.elementInActiveFormattingElements = function (name) {
        const els = this.activeFormattingElements;
        for (let i = els.length - 1; i >= 0; i--) {
      ...
      TreeBuilder.prototype.parseError = function (code, args) {
        ...
        const message = formatMessage(messages[code], args);
      ...
      TreeBuilder.prototype.adjustForeignAttributes = function (attributes) {
        for (let i = 0; i < attributes.length; i++) {
          const attribute = attributes[i];
          const adjusted = constants.ForeignAttributeMap[attribute.nodeName];
          if (adjusted) {
      ...
      function formatMessage(format, args) {
        return format.replace(new RegExp('{[0-9a-z-]+}', 'gi'), match => args[match.slice(1, -1)] || match);
      }
    ...
    9: [function (_dereq_, module, exports) {
      const SAXTreeBuilder = _dereq_('./SAXTreeBuilder').SAXTreeBuilder;
      const Tokenizer = _dereq_('../Tokenizer').Tokenizer;
      const TreeParser = _dereq_('./TreeParser').TreeParser;
      ...
      SAXParser.prototype.parse = function (source) {
        ...
        const document = this._treeBuilder.document;
        if (document) {
      ...
      SAXParser.prototype.parseFragment = function (source, context) {
        ...
        this._tokenizer.tokenize(source);
        const fragment = this._treeBuilder.getFragment();
        if (fragment) {
    ...
    10: [function (_dereq_, module, exports) {
      ...
      const TreeBuilder = _dereq_('../TreeBuilder').TreeBuilder;
      ...
      SAXTreeBuilder.prototype.insertText = function (data) {
        if (this.redirectAttachToFosterParent && this.openElements.top.isFosterParenting()) {
          const tableIndex = this.openElements.findIndex('table');
          const tableItem = this.openElements.item(tableIndex);
          const table = tableItem.node;
          if (tableIndex === 0) {
          ...
          const stackParent = this.openElements.item(tableIndex - 1).node;
      ...
      Object.defineProperty(Node.prototype, 'previousSibling', {
        get() {
          let prev = null;
          let next = this.parentNode.firstChild;
          for (;;) {
            if (this == next) {
              return prev;
            }
      ...
      Element.prototype.visit = function (treeParser) {
        if (this.prefixMappings) {
          for (const key in prefixMappings) {
            const mapping = prefixMappings[key];
...
ace.define('ace/mode/html_worker', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const lang = require('../lib/lang');
  const Mirror = require('../worker/mirror').Mirror;
  const SAXParser = require('./html/saxparser').SAXParser;
