ace.define('ace/occur', [], (require, exports, module) => {
  ...
  (function () {
    this.enter = function (editor, options) {
      ...
      const translatedPos = this.originalToOccurPosition(editor.session, pos);
      editor.moveCursorToPosition(translatedPos);
      return true;
    };
    this.exit = function (editor, options) {
      const pos = options.translatePosition && editor.getCursorPosition();
      const translatedPos = pos && this.occurToOriginalPosition(editor.session, pos);
    ...
    this.highlight = function (sess, regexp) {
      const hl = sess.$occurHighlight = sess.$occurHighlight || sess.addDynamicMarker(new SearchHighlight(null, 'ace_occur-highlight', 'text'));
      hl.setRegexp(regexp);
    ...
    this.displayOccurContent = function (editor, options) {
      ...
      const found = this.matchingLines(editor.session, options);
      const lines = found.map(foundLine => foundLine.content);
      const occurSession = new EditSession(lines.join('\n'));
    ...
    this.originalToOccurPosition = function (session, pos) {
      const lines = session.$occurMatchingLines;
    ...
    this.matchingLines = function (session, options) {
      ...
      search.set(options);
      return search.findAll(session).reduce((lines, range) => {
        const row = range.start.row;
        const last = lines[lines.length - 1];
        return last && last.row === row ?
          lines :
          lines.concat({ row, content: session.getLine(row) });
      }, []);
    };
...
ace.define('ace/commands/occur_commands', [], (require, exports, module) => {
  let config = require('../config'),
    Occur = require('../occur').Occur;
  const occurStartCommand = {
    name: 'occur',
    exec(editor, options) {
      const alreadyInOccur = !!editor.session.$occur;
      const occurSessionActive = new Occur().enter(editor, options);
  ...
  const occurCommands = [{
    ...
    bindKey: 'esc|Ctrl-G',
    exec(editor) {
      const occur = editor.session.$occur;
      if (!occur) return;
      occur.exit(editor, {});
  ...
  (function () {
    ...
    const handleKeyboard$super = this.handleKeyboard;
    this.handleKeyboard = function (data, hashId, key, keyCode) {
      const cmd = handleKeyboard$super.call(this, data, hashId, key, keyCode);
...
ace.define('ace/commands/incremental_search_commands', [], (require, exports, module) => {
  const config = require('../config');
  const oop = require('../lib/oop');
  const HashHandler = require('../keyboard/hash_handler').HashHandler;
  const occurStartCommand = require('./occur_commands').occurStartCommand;
  exports.iSearchStartCommands = [{
    name: 'iSearch',
    bindKey: { win: 'Ctrl-F', mac: 'Command-F' },
    exec(editor, options) {
      config.loadModule(['core', 'ace/incremental_search'], (e) => {
        const iSearch = e.iSearch = e.iSearch || new e.IncrementalSearch();
    ...
    exec(editor, jumpToNext) { editor.execCommand('iSearch', { backwards: true }); },
    readOnly: true,
  }, {
    name: 'iSearchAndGo',
    bindKey: { win: 'Ctrl-K', mac: 'Command-G' },
    exec(editor, jumpToNext) { editor.execCommand('iSearch', { jumpToFirstMatch: true, useCurrentOrPrevSearch: true }); },
    readOnly: true,
  }, {
    name: 'iSearchBackwardsAndGo',
    bindKey: { win: 'Ctrl-Shift-K', mac: 'Command-Shift-G' },
    exec(editor) { editor.execCommand('iSearch', { jumpToFirstMatch: true, backwards: true, useCurrentOrPrevSearch: true }); },
    readOnly: true,
  }];
  exports.iSearchCommands = [{
    ...
    bindKey: 'space',
    exec(iSearch) { iSearch.addString(' '); },
  }, {
    ...
    name: 'confirmSearch',
    bindKey: 'return',
    exec(iSearch) { iSearch.deactivate(); },
  }, {
    name: 'cancelSearch',
    bindKey: 'esc|Ctrl-G',
    exec(iSearch) { iSearch.deactivate(true); },
  }, {
    name: 'occurisearch',
    bindKey: 'Ctrl-O',
    exec(iSearch) {
      const options = oop.mixin({}, iSearch.$options);
      iSearch.deactivate();
    ...
    bindKey: 'Ctrl-w',
    exec(iSearch) {
      let ed = iSearch.$editor,
        range = ed.selection.getRangeOfMovements((sel) => { sel.moveCursorWordRight(); }),
    ...
    exec(iSearch) {
      let ed = iSearch.$editor,
        range = ed.selection.getRangeOfMovements((sel) => { sel.moveCursorRight(); }),
        string = ed.session.getTextRange(range);
      iSearch.addString(string);
    },
  }, {
    ...
    bindKey: 'Ctrl-l',
    exec(iSearch) { iSearch.$editor.execCommand('recenterTopBottom'); },
  }, {
    ...
    exec(iSearch) {
      let ed = iSearch.$editor,
        ...
        ranges = hl && hl.cache ? hl.cache
          .reduce((ranges, ea) => ranges.concat(ea || []), []) : [];
  ...
  (function () {
    this.attach = function (editor) {
      const iSearch = this.$iSearch;
      HashHandler.call(this, exports.iSearchCommands, editor.commands.platform);
      this.$commandExecHandler = editor.commands.addEventListener('exec', (e) => {
        if (!e.command.isIncrementalSearchCommand) { return iSearch.deactivate(); }
        e.stopPropagation();
        e.preventDefault();
        const scrollTop = editor.session.getScrollTop();
        const result = e.command.exec(iSearch, e.args || {});
    ...
    this.handleKeyboard = function (data, hashId, key, keyCode) {
      ...
      if (hashId == -1) {
        const extendCmd = this.commands.extendSearchTerm;
...
ace.define('ace/incremental_search', [], (require, exports, module) => {
  const oop = require('./lib/oop');
  const Range = require('./range').Range;
  const Search = require('./search').Search;
  const SearchHighlight = require('./search_highlight').SearchHighlight;
  const iSearchCommandModule = require('./commands/incremental_search_commands');
  const ISearchKbd = iSearchCommandModule.IncrementalSearchKeyboardHandler;
  ...
  (function () {
    ...
    this.addString = function (s) {
      return this.highlightAndFindWithNeedle(false, (needle) => {
        if (!isRegExp(needle)) { return needle + s; }
        const reObj = regExpToObject(needle);
        reObj.expression += s;
    ...
    this.convertNeedleToRegExp = function () {
      return this.highlightAndFindWithNeedle(false, needle => (isRegExp(needle) ? needle : stringToRegExp(needle, 'ig')));
    };

    this.convertNeedleToString = function () {
      return this.highlightAndFindWithNeedle(false, needle => (isRegExp(needle) ? regExpToObject(needle).expression : needle));
    };
...
.ace_dark div.ace_isearch-result {\
  ...
  const commands = require('./commands/command_manager');
  (function () {
    this.setupIncrementalSearch = function (editor, val) {
      if (this.usesIncrementalSearch == val) return;
      this.usesIncrementalSearch = val;
      const iSearchCommands = iSearchCommandModule.iSearchStartCommands;
      const method = val ? 'addCommands' : 'removeCommands';
...
ace.define('ace/keyboard/emacs', [], (require, exports, module) => {
  const dom = require('../lib/dom');
  require('../incremental_search');
  const iSearchCommandModule = require('../commands/incremental_search_commands');


  const HashHandler = require('./hash_handler').HashHandler;
  ...
  exports.handler.attach = function (editor) {
    ...
    editor.pushEmacsMark = function (p, activate) {
      const prevMark = this.session.$emacsMark;
  ...
  const keys = require('../lib/keys').KEY_MODS;
  const eMods = {
    C: 'ctrl', S: 'shift', M: 'alt', CMD: 'command',
  };
  ...
  exports.handler.bindKey = function (key, command) {
    ...
    key.split('|').forEach((keyPart) => {
      ...
      ckb[keyPart] = command;
      const keyParts = keyPart.split(' ').slice(0, -1);
      keyParts.reduce((keyMapKeys, keyPart, i) => {
        const prefix = keyMapKeys[i - 1] ? `${keyMapKeys[i - 1]} ` : '';
  ...
  exports.handler.handleKeyboard = function (data, hashId, key, keyCode) {
    ...
    if (hashId == -1) {
      ...
      if (data.count) {
        const str = new Array(data.count + 1).join(key);
        data.count = null;
        return { command: 'insertstring', args: str };
      }
    }

    const modifier = eMods[hashId];
    ...
    if (data.count) {
      ...
      if (!command || !command.handlesCount) {
        return {
          ...
          command: {
            exec(editor, args) {
              for (let i = 0; i < count; i++) { command.exec(editor, args); }
            },
  ...
  exports.handler.addCommands({
    recenterTopBottom(editor) {
      ...
      const pos = renderer.$cursorLayer.getPixelPosition();
      const h = renderer.$size.scrollerHeight - renderer.lineHeight;
      let scrollTop = renderer.scrollTop;
    ...
    selectRectangularRegion(editor) {
    ...
    setMark: {
      exec(editor, args) {
        ...
        let mark = editor.emacsMark(),
          ranges = editor.selection.getAllRanges(),
          rangePositions = ranges.map(r => ({ row: r.start.row, column: r.start.column })),
          transientMarkModeActive = true,
          hasNoSelection = ranges.every(range => range.isEmpty());
        ...
        if (!mark) {
          rangePositions.forEach((pos) => { editor.pushEmacsMark(pos); });
          editor.setEmacsMark(rangePositions[rangePositions.length - 1]);
        }
        function moveToMark() {
          const mark = editor.popEmacsMark();
    ...
    exchangePointAndMark: {
      exec: function exchangePointAndMark$exec(editor, args) {
        ...
        if (args.count) { // replace mark and point
          const pos = { row: sel.lead.row, column: sel.lead.column };
          sel.clearSelection();
    ...
    killLine(editor) {
      ...
      editor.clearSelection();
      const range = editor.getSelectionRange();
      let line = editor.session.getLine(range.start.row);
      range.end.column = line.length;
      line = line.substr(range.start.column);

      const foldLine = editor.session.getFoldLine(range.start.row);
    ...
    killRingSave: {
      exec(editor) {
        editor.$handlesEmacsOnCopy = true;
        let marks = editor.session.$emacsMarkRing.slice(),
          deselectedMarks = [];
        exports.killRing.add(editor.getCopyText());
...
(function () {
  ace.require(['ace/keyboard/emacs'], (m) => {
