ace.define('ace/keyboard/vim', [], (require, exports, module) => {
  function log() {
    let d = '';
    function format(p) {
  ...
  (function () {
    ...
    this.signal = function (eventName, e, handlers) {
      ...
      for (let i = 0; i < listeners.length; i++) { listeners[i](this, e); }
    };
    ...
    this.listSelections = function (p) {
      ...
      if (!ranges.length || this.ace.inVirtualSelectionMode) { return [{ anchor: this.getCursor('anchor'), head: this.getCursor('head') }]; }
    ...
    const Marker = function (cm, id, row, column) {
      this.cm = cm;
      this.id = id;
      this.row = row;
      this.column = column;
    ...
    this.findPosV = function (start, amount, unit, goalColumn) {
      if (unit == 'page') {
        ...
        amount *= Math.floor(config.height / config.lineHeight);
        unit = 'line';
      }
      if (unit == 'line') {
    ...
    this.replaceSelections = function (p) {
      ...
      for (let i = ranges.length; i--;) { this.ace.session.replace(ranges[i], p[i] || ''); }
...
.ace_dialog input {\
  ...
  const defaultKeymap = [
    ...
    },
    {
      ...
      motion: 'moveByWords',
      motionArgs: {
        forward: true, wordEnd: true, bigWord: true, inclusive: true,
      },
    },
    {
    ...
    },
    {
      ...
      motion: 'moveByWords',
      motionArgs: {
        forward: false, wordEnd: true, bigWord: true, inclusive: true,
      },
    },
    {
    ...
    },
    {
      ...
      motionArgs: {
        forward: false, explicitRepeat: true, linewise: true, toJumplist: true,
      },
    },
    {
      keys: 'G',
      ...
      motion: 'moveToLineOrEdgeOfDocument',
      motionArgs: {
        forward: true, explicitRepeat: true, linewise: true, toJumplist: true,
      },
    },
    ...
    { keys: '?', type: 'search', searchArgs: { forward: false, querySrc: 'prompt', toJumplist: true } },
    {
      ...
      type: 'search',
      searchArgs: {
        forward: true, querySrc: 'wordUnderCursor', wholeWordOnly: true, toJumplist: true,
      },
    },
    {
      keys: '#',
      type: 'search',
      searchArgs: {
        forward: false, querySrc: 'wordUnderCursor', wholeWordOnly: true, toJumplist: true,
      },
    },
  ...
  let Vim = function () { return vimApi; }; // {
  ...
  const modifiers = {
    Shift: 'S', Ctrl: 'C', Alt: 'A', Cmd: 'D', Mod: 'A',
  };
  const specialKeys = {
  ...
  const numberRegex = /[\d]/;
  let wordCharTest = [CodeMirror.isWordChar, function (ch) {
  ...
  var commandDispatcher = {
    ...
    evalInput(cm, vim) {
      ...
      if (operator) {
        ...
        const operatorMoveTo = operators[operator](cm, operatorArgs, cmSel.ranges, oldAnchor, newHead);
        if (vim.visualMode) {
  ...
  var motions = {
    ...
    moveByWords(cm, head, motionArgs) {
      return moveToWord(
        cm, head, motionArgs.repeat, !!motionArgs.forward,
  ...
  var operators = {
    ...
    delete(cm, args, ranges) {
      ...
      const includeLineBreak = vim.insertMode;
  ...
  var actions = {
    ...
    joinLines(cm, actionArgs, vim) {
      ...
      if (vim.visualMode) {
        ...
        curEnd = clipCursorToContent(cm, Pos(
  ...
  function makePrompt(prefix, desc) {
    let raw = `<span style="font-family: monospace; white-space: pre">${
  ...
  var exCommands = {
    ...
    sort(cm, params) {
      ...
      const numberRegex = pattern || ((number == 'decimal') ? /(-?)([\d]+)/ :
  ...
  function exitInsertMode(cm) {
    ...
    var text = [];
    if (!isPlaying) {
      const selLength = lastChange.inVisualBlock && vim.lastSelection ?
  ...
  function mapCommand(keys, type, name, args, extra) {
    ...
    for (const key in extra) { command[key] = extra[key]; }
    _mapCommand(command);
  }
  ...
  const handleKey = Vim.handleKey.bind(Vim);
  ...
  exports.handler = {
    ...
    handleKeyboard(data, hashId, key, keyCode, e) {
      ...
      if (key == 'esc' && !vim.insertMode && !vim.visualMode && !cm.ace.inMultiSelectMode) {
        const searchState = getSearchState(cm);
        const overlay = searchState.getOverlay();
    ...
    attach(editor) {
      ...
      function updateInputMode() {
        const isIntsert = getVim(cm).insertMode;
    ...
    updateMacCompositionHandlers(editor, enable) {
      ...
      } else if (editor.onCompositionUpdateOrig) {
...
(function () {
  ace.require(['ace/keyboard/vim'], (m) => {
