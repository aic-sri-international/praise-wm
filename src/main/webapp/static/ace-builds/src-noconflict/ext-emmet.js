ace.define('ace/snippets', [], (require, exports, module) => {
    ...
    this.insertSnippetForSelection = function (editor, snippetText) {
      ...
      tokens.forEach((t) => {
        if (typeof t === 'string') {
          ...
          text += t;
        } else if (!t.start) { t.start = { row, column }; } else { t.end = { row, column }; }
      });
...
ace.define('ace/ext/emmet', [], (require, exports, module) => {
  ...
  AceEmmetEditor.prototype = {
    setupContext(editor) {
      ...
      const resources = emmet.resources || emmet.require('resources');
    ...
    $updateTabstops(value) {
      ...
      let zeroBase = 0;
      let lastZero = null;
      const ts = emmet.tabStops || emmet.require('tabStops');
      const resources = emmet.resources || emmet.require('resources');
      const settings = resources.getVocabulary('user');
      var tabstopOptions = {
        tabstop(data) {
      ...
      if (settings.variables.insert_final_tabstop && !/\$\{0\}$/.test(value)) {
        ...
        const common = emmet.utils ? emmet.utils.common : emmet.require('utils');
  ...
  exports.runEmmetCommand = function runEmmetCommand(editor) {
    try {
      ...
      const actions = emmet.actions || emmet.require('actions');
