ace.define('ace/ext/whitespace', [], (require, exports, module) => {
  ...
  exports.$detectIndentation = function (lines, fallback) {
    ...
    function getScore(indent) {
      let score = 0;
      for (let i = indent; i < stats.length; i += indent) { score += stats[i] || 0; }
      return score;
    }
    ...
    for (var i = 1; i < 12; i++) {
      ...
      if (score > first.score) { first = { score, length: i }; }
    }
  ...
  exports.trimTrailingSpace = function (session, options) {
    ...
    const min = options && options.trimEmpty ? -1 : 0;
    let cursors = [],
      ci = -1;
    if (options && options.keepCursorPosition) {
      if (session.selection.rangeCount) {
        session.selection.rangeList.ranges.forEach((x, i, ranges) => {
    ...
    let cursorRow = cursors[ci] && cursors[ci].row;
