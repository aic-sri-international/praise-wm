...
ace.define('ace/mode/xml_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const TextHighlightRules = require('./text_highlight_rules').TextHighlightRules;
...
ace.define('ace/mode/html_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const lang = require('../lib/lang');
  const CssHighlightRules = require('./css_highlight_rules').CssHighlightRules;
...
ace.define('ace/mode/ruby_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const TextHighlightRules = require('./text_highlight_rules').TextHighlightRules;
  const constantOtherSymbol = exports.constantOtherSymbol = {
    token: 'constant.other.symbol.ruby', // symbol
    regex: '[:](?:[A-Za-z_]|[@$](?=[a-zA-Z0-9_]))[a-zA-Z0-9_]*[!=?]?',
  };

  const qString = exports.qString = {
    token: 'string', // single line
    regex: "['](?:(?:\\\\.)|(?:[^'\\\\]))*?[']",
  };

  const qqString = exports.qqString = {
    token: 'string', // single line
    regex: '["](?:(?:\\\\.)|(?:[^"\\\\]))*?["]',
  };

  const tString = exports.tString = {
    token: 'string', // backtick string
    regex: "[`](?:(?:\\\\.)|(?:[^'\\\\]))*?[`]",
  };

  const constantNumericHex = exports.constantNumericHex = {
    token: 'constant.numeric', // hex
    regex: '0[xX][0-9a-fA-F](?:[0-9a-fA-F]|_(?=[0-9a-fA-F]))*\\b',
  };

  const constantNumericFloat = exports.constantNumericFloat = {
    token: 'constant.numeric', // float
    regex: '[+-]?\\d(?:\\d|_(?=\\d))*(?:(?:\\.\\d(?:\\d|_(?=\\d))*)?(?:[eE][+-]?\\d+)?)?\\b',
  };

  const instanceVariable = exports.instanceVariable = {
    token: 'variable.instance', // instance variable
    regex: '@{1,2}[a-zA-Z_\\d]+',
  };

  const RubyHighlightRules = function () {
...
ace.define('ace/mode/html_ruby_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const HtmlHighlightRules = require('./html_highlight_rules').HtmlHighlightRules;
  const RubyHighlightRules = require('./ruby_highlight_rules').RubyHighlightRules;

  const HtmlRubyHighlightRules = function () {
    ...
    for (const key in this.$rules) { this.$rules[key].unshift.apply(this.$rules[key], startRules); }
...
ace.define('ace/mode/javascript', [], (require, exports, module) => {
  const oop = require('../lib/oop');
...
ace.define('ace/mode/css_completions', [], (require, exports, module) => {
  const propertyMap = {
    background: { '#$0': 1 },
    'background-color': { '#$0': 1, transparent: 1, fixed: 1 },
    'background-image': { "url('/$0')": 1 },
    'background-repeat': {
      repeat: 1, 'repeat-x': 1, 'repeat-y': 1, 'no-repeat': 1, inherit: 1,
    },
    'background-position': {
      bottom: 2, center: 2, left: 2, right: 2, top: 2, inherit: 2,
    },
    ...
    border: {
      'solid $0': 1, 'dashed $0': 1, 'dotted $0': 1, '#$0': 1,
    },
    'border-color': { '#$0': 1 },
    'border-style': {
      solid: 2, dashed: 2, dotted: 2, double: 2, groove: 2, hidden: 2, inherit: 2, inset: 2, none: 2, outset: 2, ridged: 2,
    },
    'border-collapse': { collapse: 1, separate: 1 },
    bottom: { px: 1, em: 1, '%': 1 },
    clear: {
      left: 1, right: 1, both: 1, none: 1,
    },
    color: { '#$0': 1, 'rgb(#$00,0,0)': 1 },
    cursor: {
      default: 1, pointer: 1, move: 1, text: 1, wait: 1, help: 1, progress: 1, 'n-resize': 1, 'ne-resize': 1, 'e-resize': 1, 'se-resize': 1, 's-resize': 1, 'sw-resize': 1, 'w-resize': 1, 'nw-resize': 1,
    },
    display: {
      none: 1, block: 1, inline: 1, 'inline-block': 1, 'table-cell': 1,
    },
    'empty-cells': { show: 1, hide: 1 },
    float: { left: 1, right: 1, none: 1 },
    'font-family': {
      Arial: 2, 'Comic Sans MS': 2, Consolas: 2, 'Courier New': 2, Courier: 2, Georgia: 2, Monospace: 2, 'Sans-Serif': 2, 'Segoe UI': 2, Tahoma: 2, 'Times New Roman': 2, 'Trebuchet MS': 2, Verdana: 1,
    },
    ...
    'list-style-type': {
      none: 1, disc: 1, circle: 1, square: 1, decimal: 1, 'decimal-leading-zero': 1, 'lower-roman': 1, 'upper-roman': 1, 'lower-greek': 1, 'lower-latin': 1, 'upper-latin': 1, georgian: 1, 'lower-alpha': 1, 'upper-alpha': 1,
    },
    margin: { px: 1, em: 1, '%': 1 },
    ...
    overflow: {
      hidden: 1, visible: 1, auto: 1, scroll: 1,
    },
    'overflow-x': {
      hidden: 1, visible: 1, auto: 1, scroll: 1,
    },
    ...
    position: {
      absolute: 1, relative: 1, fixed: 1, static: 1,
    },
    right: { px: 1, em: 1, '%': 1 },
    'table-layout': { fixed: 1, auto: 1 },
    'text-decoration': {
      none: 1, underline: 1, 'line-through': 1, blink: 1,
    },
    'text-align': {
    ...
    'white-space': {
      nowrap: 1, normal: 1, pre: 1, 'pre-line': 1, 'pre-wrap': 1,
    },
    width: { px: 1, em: 1, '%': 1 },
...
ace.define('ace/mode/behaviour/css', [], (require, exports, module) => {
  const oop = require('../../lib/oop');
  const Behaviour = require('../behaviour').Behaviour;
  const CstyleBehaviour = require('./cstyle').CstyleBehaviour;
...
ace.define('ace/mode/css', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const TextMode = require('./text').Mode;
  const CssHighlightRules = require('./css_highlight_rules').CssHighlightRules;
  const MatchingBraceOutdent = require('./matching_brace_outdent').MatchingBraceOutdent;
  const WorkerClient = require('../worker/worker_client').WorkerClient;
  const CssCompletions = require('./css_completions').CssCompletions;
  const CssBehaviour = require('./behaviour/css').CssBehaviour;
  const CStyleFoldMode = require('./folding/cstyle').FoldMode;
...
ace.define('ace/mode/behaviour/xml', [], (require, exports, module) => {
  const oop = require('../../lib/oop');
  const Behaviour = require('../behaviour').Behaviour;
  const TokenIterator = require('../../token_iterator').TokenIterator;
  const lang = require('../../lib/lang');
...
ace.define('ace/mode/folding/mixed', [], (require, exports, module) => {
  const oop = require('../../lib/oop');
  const BaseFoldMode = require('./fold_mode').FoldMode;
...
ace.define('ace/mode/folding/xml', [], (require, exports, module) => {
  const oop = require('../../lib/oop');
  const lang = require('../../lib/lang');
  const Range = require('../../range').Range;
  const BaseFoldMode = require('./fold_mode').FoldMode;
...
ace.define('ace/mode/folding/html', [], (require, exports, module) => {
  const oop = require('../../lib/oop');
...
ace.define('ace/mode/html_completions', [], (require, exports, module) => {
  const TokenIterator = require('../token_iterator').TokenIterator;
  ...
  const attributeMap = {
    a: {
      ...
      ping: 1,
      rel: {
        nofollow: 1, alternate: 1, author: 1, bookmark: 1, help: 1, license: 1, next: 1, noreferrer: 1, prefetch: 1, prev: 1, search: 1, tag: 1,
      },
      media: 1,
      hreflang: 1,
      type: 1,
    ...
    address: {},
    area: {
      shape: 1, coords: 1, href: 1, hreflang: 1, alt: 1, target: 1, media: 1, rel: 1, ping: 1, type: 1,
    },
    article: { pubdate: 1 },
    aside: {},
    audio: {
      src: 1, autobuffer: 1, autoplay: { autoplay: 1 }, loop: { loop: 1 }, controls: { controls: 1 }, muted: { muted: 1 }, preload: { auto: 1, metadata: 1, none: 1 },
    },
    b: {},
    ...
    blockquote: { cite: 1 },
    body: {
      onafterprint: 1, onbeforeprint: 1, onbeforeunload: 1, onhashchange: 1, onmessage: 1, onoffline: 1, onpopstate: 1, onredo: 1, onresize: 1, onstorage: 1, onundo: 1, onunload: 1,
    },
    br: {},
    button: {
      autofocus: 1, disabled: { disabled: 1 }, form: 1, formaction: 1, formenctype: 1, formmethod: 1, formnovalidate: 1, formtarget: 1, name: 1, value: 1, type: { button: 1, submit: 1 },
    },
    ...
    colgroup: { span: 1 },
    command: {
      type: 1, label: 1, icon: 1, disabled: 1, checked: 1, radiogroup: 1, command: 1,
    },
    data: {},
    datalist: {},
    dd: {},
    ...
    footer: {},
    form: {
      'accept-charset': 1, action: 1, autocomplete: 1, enctype: { 'multipart/form-data': 1, 'application/x-www-form-urlencoded': 1 }, method: { get: 1, post: 1 }, name: 1, novalidate: 1, target: { _blank: 1, top: 1 },
    },
    h1: {},
    h2: {},
    h3: {},
    ...
    iframe: {
      ...
      width: 1,
      sandbox: {
        'allow-same-origin': 1, 'allow-top-navigation': 1, 'allow-forms': 1, 'allow-scripts': 1,
      },
      seamless: { seamless: 1 },
    },
    img: {
      alt: 1, src: 1, height: 1, width: 1, usemap: 1, ismap: 1,
    },
    input: {
      type: {
        text: 1, password: 1, hidden: 1, checkbox: 1, submit: 1, radio: 1, file: 1, button: 1, reset: 1, image: 31, color: 1, date: 1, datetime: 1, 'datetime-local': 1, email: 1, month: 1, number: 1, range: 1, search: 1, tel: 1, time: 1, url: 1, week: 1,
      },
      accept: 1,
      alt: 1,
    ...
    kbd: {},
    keygen: {
      autofocus: 1, challenge: { challenge: 1 }, disabled: { disabled: 1 }, form: 1, keytype: { rsa: 1, dsa: 1, ec: 1 }, name: 1,
    },
    label: { form: 1, for: 1 },
    legend: {},
    li: { value: 1 },
    link: {
      ...
      type: {
        'text/css': 1, 'image/png': 1, 'image/jpeg': 1, 'image/gif': 1,
      },
      sizes: 1,
    },
    main: {},
    ...
    menu: { type: 1, label: 1 },
    meta: {
      'http-equiv': { 'content-type': 1 }, name: { description: 1, keywords: 1 }, content: { 'text/html; charset=UTF-8': 1 }, charset: 1,
    },
    meter: {
      value: 1, min: 1, max: 1, low: 1, high: 1, optimum: 1,
    },
    nav: {},
    noscript: { href: 1 },
    object: {
      param: 1, data: 1, type: 1, height: 1, width: 1, usemap: 1, name: 1, form: 1, classid: 1,
    },
    ol: { start: 1, reversed: 1 },
    optgroup: { disabled: 1, label: 1 },
    option: {
      disabled: 1, selected: 1, label: 1, value: 1,
    },
    ...
    samp: {},
    script: {
      charset: 1, type: { 'text/javascript': 1 }, src: 1, defer: 1, async: 1,
    },
    select: {
      autofocus: 1, disabled: 1, form: 1, multiple: { multiple: 1 }, name: 1, size: 1, readonly: { readonly: 1 },
    },
    small: {},
    ...
    textarea: {
      ...
      cols: 1,
      wrap: {
        on: 1, off: 1, hard: 1, soft: 1,
      },
    },
    tfoot: {},
    th: {
      headers: 1, rowspan: 1, colspan: 1, scope: 1,
    },
    thead: {},
    time: { datetime: 1 },
    title: {},
    tr: {},
    track: {
      kind: 1, src: 1, srclang: 1, label: 1, default: 1,
    },
    section: {},
    summary: {},
    u: {},
    ...
    var: {},
    video: {
      src: 1, autobuffer: 1, autoplay: { autoplay: 1 }, loop: { loop: 1 }, controls: { controls: 1 }, width: 1, height: 1, poster: 1, muted: { muted: 1 }, preload: { auto: 1, metadata: 1, none: 1 },
    },
    wbr: {},
  };
...
ace.define('ace/mode/html', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const lang = require('../lib/lang');
  const TextMode = require('./text').Mode;
  ...
  const HtmlCompletions = require('./html_completions').HtmlCompletions;
  const WorkerClient = require('../worker/worker_client').WorkerClient;
...
ace.define('ace/mode/folding/coffee', [], (require, exports, module) => {
  const oop = require('../../lib/oop');
  const BaseFoldMode = require('./fold_mode').FoldMode;
  const Range = require('../../range').Range;
...
ace.define('ace/mode/ruby', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const TextMode = require('./text').Mode;
  const RubyHighlightRules = require('./ruby_highlight_rules').RubyHighlightRules;
  const MatchingBraceOutdent = require('./matching_brace_outdent').MatchingBraceOutdent;
  ...
  (function () {
    ...
    this.getNextLineIndent = function (state, line, tab) {
      ...
      if (state == 'start') {
        const match = line.match(/^.*[\{\(\[]\s*$/);
        const startingClassOrMethod = line.match(/^\s*(class|def|module)\s.*$/);
        const startingDoBlock = line.match(/.*do(\s*|\s+\|.*\|\s*)$/);
        const startingConditional = line.match(/^\s*(if|else|when)\s*/);
...
ace.define('ace/mode/html_ruby', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const HtmlRubyHighlightRules = require('./html_ruby_highlight_rules').HtmlRubyHighlightRules;
  const HtmlMode = require('./html').Mode;
  const JavaScriptMode = require('./javascript').Mode;
  const CssMode = require('./css').Mode;
  const RubyMode = require('./ruby').Mode;
...
(function () {
  ace.require(['ace/mode/html_ruby'], (m) => {
