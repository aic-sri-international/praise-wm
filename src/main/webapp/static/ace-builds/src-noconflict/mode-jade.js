...
ace.define('ace/mode/markdown_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const lang = require('../lib/lang');
  const TextHighlightRules = require('./text_highlight_rules').TextHighlightRules;
  const JavaScriptHighlightRules = require('./javascript_highlight_rules').JavaScriptHighlightRules;
  ...
  const MarkdownHighlightRules = function () {
...
ace.define('ace/mode/scss_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const lang = require('../lib/lang');
  const TextHighlightRules = require('./text_highlight_rules').TextHighlightRules;

  const ScssHighlightRules = function () {
    const properties = lang.arrayToMap((function () {
      const browserPrefix = ('-webkit-|-moz-|-o-|-ms-|-svg-|-pie-|-khtml-').split('|');

      const prefixProperties = ('appearance|background-clip|background-inline-policy|background-origin|' +
      ...
      const properties = ('azimuth|background-attachment|background-color|background-image|' +
            ...
            'z-index').split('|');
      const ret = [];
      for (let i = 0, ln = browserPrefix.length; i < ln; i++) {
    ...
    const functions = lang.arrayToMap(('hsl|hsla|rgb|rgba|url|attr|counter|counters|abs|adjust_color|adjust_hue|' +
         'alpha|join|blue|ceil|change_color|comparable|complement|darken|desaturate|' +
         'floor|grayscale|green|hue|if|invert|join|length|lighten|lightness|mix|' +
         'nth|opacify|opacity|percentage|quote|red|round|saturate|saturation|' +
         'scale_color|transparentize|type_of|unit|unitless|unquote').split('|'));

    const constants = lang.arrayToMap(('absolute|all-scroll|always|armenian|auto|baseline|below|bidi-override|' +
    ...
    const colors = lang.arrayToMap(('aliceblue|antiquewhite|aqua|aquamarine|azure|beige|bisque|black|' +
    ...
    const keywords = lang.arrayToMap(('@mixin|@extend|@include|@import|@media|@debug|@warn|@if|@for|@each|@while|@else|@font-face|@-webkit-keyframes|if|and|!default|module|def|end|declare').split('|'));

    const tags = lang.arrayToMap(('a|abbr|acronym|address|applet|area|article|aside|audio|b|base|basefont|bdo|' +
      ...
      start: [
        ...
        },
        {
          ...
          token(value) {
            ...
            if (keywords.hasOwnProperty(value)) { return 'keyword'; } else if (constants.hasOwnProperty(value)) { return 'constant.language'; } else if (functions.hasOwnProperty(value)) { return 'support.function'; } else if (colors.hasOwnProperty(value.toLowerCase())) { return 'support.constant.color'; } else if (tags.hasOwnProperty(value.toLowerCase())) { return 'variable.language'; }
            return 'text';
          },
          regex: '\\-?[@a-z_][@a-z0-9_\\-]*',
        }, {
...
ace.define('ace/mode/less_highlight_rules', [], (require, exports, module) => {
  ...
  const LessHighlightRules = function () {
    const keywordList = '@import|@media|@font-face|@keyframes|@-webkit-keyframes|@supports|' +
        '@charset|@plugin|@namespace|@document|@page|@viewport|@-ms-viewport|' +
        'or|and|when|not';

    const keywords = keywordList.split('|');

    const properties = CssHighlightRules.supportType.split('|');
...
ace.define('ace/mode/coffee_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const TextHighlightRules = require('./text_highlight_rules').TextHighlightRules;
...
ace.define('ace/mode/jade_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const TextHighlightRules = require('./text_highlight_rules').TextHighlightRules;
  const MarkdownHighlightRules = require('./markdown_highlight_rules').MarkdownHighlightRules;
  const SassHighlightRules = require('./scss_highlight_rules').ScssHighlightRules;
  const LessHighlightRules = require('./less_highlight_rules').LessHighlightRules;
  const CoffeeHighlightRules = require('./coffee_highlight_rules').CoffeeHighlightRules;
  const JavaScriptHighlightRules = require('./javascript_highlight_rules').JavaScriptHighlightRules;
  ...
  const JadeHighlightRules = function () {
    const escapedRe = '\\\\(?:x[0-9a-fA-F]{2}|' + // hex
        'u[0-9a-fA-F]{4}|' + // unicode
        '[0-2][0-7]{0,2}|' + // oct
        '3[0-6][0-7]?|' + // oct
...
ace.define('ace/mode/jade', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const TextMode = require('./text').Mode;
  const JadeHighlightRules = require('./jade_highlight_rules').JadeHighlightRules;
