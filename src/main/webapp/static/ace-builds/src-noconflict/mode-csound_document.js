ace.define('ace/mode/csound_preprocessor_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');

  const TextHighlightRules = require('./text_highlight_rules').TextHighlightRules;

  const CsoundPreprocessorHighlightRules = function () {
  ...
  (function () {
    this.pushRule = function (params) {
      return {
        regex: params.regex,
        onMatch(value, currentState, stack, line) {
          if (stack.length === 0) { stack.push(currentState); }
          if (Array.isArray(params.next)) {
            for (let i = 0; i < params.next.length; i++) {
        ...
        set next(next) {
          if (Array.isArray(params.next)) {
            const oldNext = params.next[params.next.length - 1];
            let oldNextIndex = oldNext.length - 1;
            let newNextIndex = next.length - 1;
            if (newNextIndex > oldNextIndex) {
              while (oldNextIndex >= 0 && newNextIndex >= 0) {
                if (oldNext.charAt(oldNextIndex) !== next.charAt(newNextIndex)) {
                  const prefix = next.substr(0, newNextIndex);
                  for (let i = 0; i < params.next.length; i++) {
...
ace.define('ace/mode/csound_score_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');

  const CsoundPreprocessorHighlightRules = require('./csound_preprocessor_highlight_rules').CsoundPreprocessorHighlightRules;

  const CsoundScoreHighlightRules = function () {
...
ace.define('ace/mode/lua_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const TextHighlightRules = require('./text_highlight_rules').TextHighlightRules;
...
ace.define('ace/mode/python_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const TextHighlightRules = require('./text_highlight_rules').TextHighlightRules;
...
ace.define('ace/mode/csound_orchestra_highlight_rules', [], (require, exports, module) => {
  const lang = require('../lib/lang');
  const oop = require('../lib/oop');

  const CsoundPreprocessorHighlightRules = require('./csound_preprocessor_highlight_rules').CsoundPreprocessorHighlightRules;
  const CsoundScoreHighlightRules = require('./csound_score_highlight_rules').CsoundScoreHighlightRules;
  ...
  const CsoundOrchestraHighlightRules = function () {
    CsoundPreprocessorHighlightRules.call(this);
    let opcodes = [
      'ATSadd',
      'ATSaddnz',
...
ace.define('ace/mode/csound_document_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');

  const CsoundOrchestraHighlightRules = require('./csound_orchestra_highlight_rules').CsoundOrchestraHighlightRules;
  const CsoundScoreHighlightRules = require('./csound_score_highlight_rules').CsoundScoreHighlightRules;
  const HtmlHighlightRules = require('./html_highlight_rules').HtmlHighlightRules;
  const TextHighlightRules = require('./text_highlight_rules').TextHighlightRules;

  const CsoundDocumentHighlightRules = function () {
    this.$rules = {
      start: [
        {
...
ace.define('ace/mode/csound_document', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const TextMode = require('./text').Mode;
  const CsoundDocumentHighlightRules = require('./csound_document_highlight_rules').CsoundDocumentHighlightRules;
...
(function () {
  ace.require(['ace/mode/csound_document'], (m) => {
