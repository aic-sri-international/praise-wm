ace.define('ace/ext/menu_tools/overlay_page', [], (require, exports, module) => {
...
ace.define('ace/ext/modelist', [], (require, exports, module) => {
  const modes = [];
  ...
  const Mode = function (name, caption, extensions) {
    this.name = name;
...
ace.define('ace/ext/themelist', [], (require, exports, module) => {
...
ace.define('ace/ext/options', [], (require, exports, module) => {
  ...
  const modes = modelist.modes.map(x => ({ caption: x.caption, value: x.mode }));
  ...
  const OptionPanel = function (editor, element) {
    this.editor = editor;
  ...
  (function () {
    ...
    this.renderOptionGroup = function (group) {
      return Object.keys(group).map((key, i) => {
        ...
        return item;
      }).sort((a, b) => a.position - b.position).map(function (item) {
    ...
    this.renderOptionControl = function (key, option) {
      ...
      if (Array.isArray(option)) {
        return option.map(x => self.renderOptionControl(key, x));
      }
      let control;
      ...
      if (option.values && option.type != 'checkbox') {
        ...
        option.items = option.values.map(v => ({ value: v, name: v }));
      }

      if (option.type == 'buttonBar') {
        control = ['div', option.items.map(item => ['button', {
          value: item.value,
        ...
        if (option.defaults) {
          control = [control, option.defaults.map(item => ['button', {
            onclick() {
        ...
        const buildItems = function (items) {
          return items.map(item => ['option', { value: item.value || item.name }, item.desc || item.caption || item.name]);
        };

        const items = Array.isArray(option.items)
          ? buildItems(option.items)
          : Object.keys(option.items).map(key => ['optgroup', { label: key }, buildItems(option.items[key])]);
        control = ['select', {
          id: key,
          value,
          onchange() {
    ...
    this.setOption = function (option, value) {
      ...
      if (option.onchange) { option.onchange(value); } else { this.editor.setOption(option.path, value); }
...
ace.define('ace/ext/settings_menu', [], (require, exports, module) => {
  const OptionPanel = require('ace/ext/options').OptionPanel;
  const overlayPage = require('./menu_tools/overlay_page').overlayPage;
  function showSettingsMenu(editor) {
    if (!document.getElementById('ace_settingsmenu')) {
      const options = new OptionPanel(editor);
      options.render();
...
(function () {
  ace.require(['ace/ext/settings_menu'], (m) => {
