...
ace.define('ace/mode/coldfusion_highlight_rules', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const JavaScriptHighlightRules = require('./javascript_highlight_rules').JavaScriptHighlightRules;
  const HtmlHighlightRules = require('./html_highlight_rules').HtmlHighlightRules;

  const ColdfusionHighlightRules = function () {
    HtmlHighlightRules.call(this);
    this.$rules.tag[2].token = function (start, tag) {
      const group = tag.slice(0, 2) == 'cf' ? 'keyword' : 'meta.tag';
      return [`meta.tag.punctuation.${start == '<' ? '' : 'end-'}tag-open.xml`,
        `${group}.tag-name.xml`];
    };

    const jsAndCss = Object.keys(this.$rules).filter(x => /^(js|css)-/.test(x));
    this.embedRules({
      cfmlComment: [
...
ace.define('ace/mode/coldfusion', [], (require, exports, module) => {
  const oop = require('../lib/oop');
  const lang = require('../lib/lang');
  const HtmlMode = require('./html').Mode;
  const ColdfusionHighlightRules = require('./coldfusion_highlight_rules').ColdfusionHighlightRules;

  const voidElements = 'cfabort|cfapplication|cfargument|cfassociate|cfbreak|cfcache|cfcollection|cfcookie|cfdbinfo|cfdirectory|cfdump|cfelse|cfelseif|cferror|cfexchangecalendar|cfexchangeconnection|cfexchangecontact|cfexchangefilter|cfexchangetask|cfexit|cffeed|cffile|cfflush|cfftp|cfheader|cfhtmlhead|cfhttpparam|cfimage|cfimport|cfinclude|cfindex|cfinsert|cfinvokeargument|cflocation|cflog|cfmailparam|cfNTauthenticate|cfobject|cfobjectcache|cfparam|cfpdfformparam|cfprint|cfprocparam|cfprocresult|cfproperty|cfqueryparam|cfregistry|cfreportparam|cfrethrow|cfreturn|cfschedule|cfsearch|cfset|cfsetting|cfthrow|cfzipparam)'.split('|');
...
(function () {
  ace.require(['ace/mode/coldfusion'], (m) => {
